...
function MusicPlayer(props: any) {
  ...
  const prevAudio = () => {
    ...
    MUSIC.loadAudio(previous.src, playing);
  }
  ...
  const nextAudio = () => {
    ...
    setPlaying(true);
    MUSIC.loadAudio(next.src, true);
  }

  const playAudio = (libraryItem : any) => {
    setCurrentlyplaying(libraryItem);
    setPlaying(true);
    MUSIC.loadAudio(libraryItem.src, true);
  }

  // Set Volume by audio.volume [min: 0, max: 1]
  const changeVolume = (e: any) => {
    setVolume(parseInt(e.target.value, 10))
    MUSIC.setVolume(parseInt(e.target.value, 10) * 0.01);
  }
  ...
  return (
    <div id="music-player" className="d-flex justify-content-center align-items-center">
      <div className="card bg-transparent">
        ...
        <Playlist show={showplaylist} playAudio={playAudio} toggle={() => { setShowplaylist(!showplaylist) }} />
      </div>
    </div>
  );
}
